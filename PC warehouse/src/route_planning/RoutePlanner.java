...
public class RoutePlanner {
	...
	public HashMap<String, ArrayList<Location>> getMultiRobotRoute(Location robot1Start, Location robot1End,
			Location robot2Start, Location robot2End, Location robot3Start, Location robot3End) {
		...
		// Priority 1/2/3
		route1 = getRoute(robot1Start, robot1End);
		reservePlace(route1);
		route2 = getRoute(robot2Start, robot2End);
		reservePlace(route2);
		route3 = getRoute(robot3Start, robot3End);
		reservePlace(route3);
		...
		// According to the last priority, the empty routes will return just
		// stop
		System.out.println("Success 7");
		if (route1.size() == 0) {
			for (int i = 1; i <= WINDOW; i++) {
				route1.add(robot1Start);
			}
			;
		} else {
		...
		if (route2.size() == 0) {
			for (int i = 1; i <= WINDOW; i++) {
				route2.add(robot2Start);
			}
			;
		} else {
			Location last = route2.get(route2.size() - 1);
			while (route2.size() < WINDOW) {
				route2.add(last);
			}
			;
		}
		...
		if (route3.size() == 0) {
			for (int i = 1; i <= WINDOW; i++) {
				route3.add(robot3Start);
			}
			;
		} else {
			Location last = route3.get(route3.size() - 1);
			while (route3.size() < WINDOW) {
				route3.add(last);
			}
			;
		}
	...
	private HashMap<String, ArrayList<Location>> createSolution(ArrayList<Location> route1, ArrayList<Location> route2,
			ArrayList<Location> route3) {
